...
## Phase 1: Persist Analysis + Auto-tag

Storage layer for saving/loading analysis results, and TUI integration to use cached analysis.

### Tasks

- [~] Task 1.1: Add `save_analysis()` and `load_analysis()` to `crates/supervox-agent/src/storage.rs`. File pattern: `{date}-{id}.analysis.json` (sibling to call JSON). `load_analysis()` returns `Option<CallAnalysis>` — None if not found.
- [ ] Task 1.2: Add `update_call_tags()` to `crates/supervox-agent/src/storage.rs` — loads call, sets `tags` from provided themes list, re-saves. Idempotent (same themes = no-op write).
- [ ] Task 1.3: Unit tests for `save_analysis()`, `load_analysis()`, `update_call_tags()` in `crates/supervox-agent/src/storage.rs` — roundtrip, not-found returns None, tag update overwrites.
...
- [ ] `cargo test --workspace` passes with new storage tests
- [ ] Opening a call twice: first time runs LLM, second time loads from disk (no LLM call)
- [ ] Call JSON has populated `tags` field after analysis
